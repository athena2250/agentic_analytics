...
**Met.** The definition above is executable rather than asserted: `agentic_analytics_kiro/tests/` holds one test class per bullet, run with `pytest` from `agentic_analytics_kiro/`. Both fixtures share no column name with each other or with the `revenue`/`department`/`customer_id` literals of §1 — an HR file (`employee_id`, `joined_on`, `team`, `annual_cost_usd`, `tenure_years`) and a marketing file (`campaign`, `week_start`, `impressions`, `clicks`, `spend_gbp`) — and every criterion is checked against both, so no assertion can pass by holding only for sales-shaped data. The local model is stubbed, so the suite needs no ollama and tests the pipeline (profiling, validation, fallback, forecasting, streaming, export) rather than a model's SQL. 54 tests pass; each bullet's tests were confirmed to fail when the behaviour they describe is removed.

Writing them found one criterion that was not actually met. Bullet 2 says the live `/query` path performs zero column-name-literal assumptions, but `_normalize` was still rewriting every question through `config.SPELL_CORRECTIONS` — a fixed sales-domain table (`revnue`→`revenue`, `departmnt`→`department`) — and lowercasing the whole query on the way through. On a dataset with none of those columns it was dead weight; on one whose column really is spelled `revnue` it corrupted the question. `api._normalize` now corrects typos against the session's own table and column names by string similarity, skips tokens that exactly match a real name, ignores tokens under four characters, and preserves the user's capitalisation. `config.py` keeps both literal tables for the legacy `app.py` CLI only, marked as such; nothing in `api.py` imports them any more.
